...
この文書は、`analyze_input_attribution.py run` が作る結果を、人に説明・報告するための読み方です。生成先は通常 `outputs/<experiment>/attribution/<output-prefix>/` です。生成物は Git 管理外で別の環境には存在しないため、この文書では結果ファイルへの直接リンクは張りません。
...
1. **offline 摂動**: ある入力または入力群を基準値に置換すると、方策の行動確率分布はどれだけ変わるか。
2. **Integrated Gradients（IG）**: 基準観測から現在観測へ移る経路上で、方策の特定の出力をどの入力が増減させたか。
3. **closed-loop**: 同じ seed の通常走行と、方策に渡す特定入力だけを置換した走行を比べると、固定方策の報酬・成功・逸脱はどう変わるか。
...
|---|---|---|---|
|1|`report.md`|設定と上位結果の全体像|ここにある順位だけで結論を確定しない|
...
|4|摂動の 2 CSV|方策分布を変えた feature / group を、同一 slice・同一 baseline で順位付けする|JSD を IG や実報酬と混ぜない|
|5|IG の 3 CSV|target ごとの寄与の向き・大きさと、数値積分の健全性を確認する|actor と critic の IG を混ぜない|
|6|`closed_loop_summary.csv` と必要な PNG|候補入力の置換が paired 走行をどう変えたかを確認する|1 scenario の結果を一般化しない|

PNG は発表用の概観、CSV は引用する数値の正本、NPZ/JSONL は監査・再集計用です。自動生成の `report.md` は読み始める助けにはなりますが、順位の根拠は必ず CSV と metadata に戻って確認します。
...
|手法|`perturbation`、`integrated_gradients.targets`、`integrated_gradients.steps`、`phases`|同じ target、積分点数、slice 定義かを確認する|
...
です。従って、`mean_delta_total_reward < 0` は介入後の総報酬が低い、`mean_delta_success = -1` はその集計対象で成功が 1 減った、`mean_delta_out_of_road = +1` は逸脱が 1 増えたことを示します。
...
|phase|metadata 上は `phases = []`|early/middle/late の 3 progress bin はあるが、曲線進入・頂点・退出の検証済み区間ではない|

`official_left_curve` は analysis config と output の**名前**です。この run には `[[phases]]` がなく、道路形状を示す地理的 annotation もありません。従って「127 decision が左カーブを走っている」「progress bin 2 がカーブ頂点である」とは言えません。名称だけを根拠に phase を報告しないでください。

曲線区間、scenario 数、baseline、解析対象などを変える場所と実行例は、[入力寄与解析ガイドの「どこを変えるか」](input_attribution.md#どこを変えるか) に集約しています。条件を変えた run は旧 run と混ぜず、metadata の hash、変更 field、新しい実験名を報告してください。

## CSV の正しい抽出条件

報告用の全 episode 要約は、次の filter を**同時に**使います。`baseline_index` が空欄の `mean_over_baselines` 行を選び、`full_episode`、`episode`、`progress_bin`、`phase` の行を一つのランキングに混ぜません。複数 baseline を使った run では、`mean_over_baselines` はまず各 decision で baseline 軸を平均してから、mean/median/p90 を集計した行です。標準 CLI は individual-baseline 行を summary CSV に出しません。baseline ごとのばらつきが必要なら raw NPZ の baseline 軸を独自集計するか、`include_baseline_rows` を設定へ露出するコード変更が必要です。
...
今の run では full episode 行の `count` はすべて **127** です。phase は未設定なので `scope = phase` の行はありません。progress を見る場合だけ `scope = progress_bin` と `slice_name = progress_bin_1_of_3` などを固定し、その bin の `count` を報告します。

CSV 行が多い理由は、同じ target を複数の時間 slice と IG target で集計しているためです。通常の出力（baseline ごとの個別行を含めない設定）では、header を除く行数は次式になります。
...
$$
\begin{aligned}
&\text{摂動 feature}=F S, &&\text{摂動 group}=G S,\\
&\text{IG feature}=F T S, &&\text{IG group}=G T S,\\
&\text{IG completeness}=T S.
\end{aligned}
$$

ここで \(F\) は feature 数、\(G\) は semantic group と LiDAR sector を合わせた数、\(T\) は IG target 数、\(E\) は episode 数、\(P\) は progress bin 数、\(H\) は明示 phase 数です。現行 run は \(F=259\)、\(G=36=12+24\)、\(T=2\)、\(E=1\)、\(P=3\)、\(H=0\) なので \(S=5\) です。実際に header を含めて、摂動 feature は 1,296 行、摂動 group は 181 行、IG feature は 2,591 行、IG group は 361 行、completeness は 11 行になります。これは重複した実験ではなく、再集計可能な slice を一つの CSV に並べた結果です。
...
|`mean_absolute_ig`|\(\operatorname{mean}|\operatorname{IG}_i|\)、target と同じ単位|符号なしの大きさ|「正の寄与」ではない。baseline/path が違う run と直接比較しない|
|`group_signed_ig`|group 内 signed IG の和を各 decision で取り、その平均|group 全体の符号付き方向|正負の打消しを含む。absolute mass と混同しない|
|`group_absolute_mass`|group 内 \(\sum_i |\operatorname{IG}_i|\) の平均|非負の大きさ|group が大きいほど増えやすい。feature と同じ順位に混ぜない|
|`normalized_absolute_mass`|LiDAR sector のみ。各 decision の LiDAR 全 absolute mass に対する比|無次元の比|同一 target・slice 内の sector 比較に限る。LiDAR 全 mass が 0 の decision は実装上 0 と記録される|
...
\qquad
r=\frac{|\varepsilon|}{\max\left(|F(x)-F(b)|,10^{-12}\right)}
$$
...
これはこの 64 点台形則の数値診断であり、IG の意味的妥当性・因果性・モデル性能の合格判定ではありません。用途横断の universal threshold はありません。steps や baseline を変えたときは、同じ target・slice でこの値の変化を確認します。
...
|PNG|表示しているもの|正しい読み方|
|---|---|---|
|`perturbation_feature_top.png`|full episode・baseline 平均の feature JSD 上位 20|actor 分布を最も変えた単一 feature の概観。`perturbation_feature_summary.csv` で値と `count` を確認する|
|`perturbation_group_top.png`|semantic group と LiDAR sector を含む JSD 上位 20|`target_kind` と `target_size` を必ず併記する。group と sector を一つの重要度順位にしない|
|`ig_feature_top_absolute.png`|最初に設定した IG target の feature `mean_absolute_ig` 上位 20|現行では actor の `selected_log_probability` のみ。critic の図ではない|
|`ig_group_top_absolute.png`|最初の IG target の semantic group / LiDAR sector `group_absolute_mass` 上位 20|`group_kind` と `group_size` を確認して大きさを読む。重複を確認し、符号は読まない|
|`ig_group_signed.png`|最初の IG target の semantic group / LiDAR sector `group_signed_ig` 上位 20（絶対値で選出）|`group_kind` を分け、0 より右/左で方向を見る。打消しがあり得るため absolute mass とセットで読む|
|`attribution_over_time.png`|最初の IG target について、baseline 平均後の signed IG の feature 次元平均絶対値を各 decision に表示|いつ大きいかの概観。特定 feature の時系列でも phase の根拠でもない|
|`perturbation_over_time.png`|各 decision の全単一 feature JSD の平均|どの decision で baseline replacement に敏感かの概観。group は含まない|
|`lidar_ig_heatmap.png`|最初の IG target の decision x LiDAR angle ごとの absolute IG|angle は schema の走査方向であり、道路座標の「前/左」を自動的には意味しない|
|`lidar_perturbation_heatmap.png`|decision x LiDAR feature ごとの JSD|sector 集計ではなく単一 LiDAR ray の置換結果。schema の index/angle を使って読む|
|`closed_loop_performance.png`|各 selected target の平均 \(\Delta\) total reward|赤の負値は介入後に悪化。成功・逸脱・scenario 数は CSV で併読する|
|`custom_feature_attribution_over_time.png`|`kind = custom` feature の最初の IG target に対する signed 時系列|現行 schema には custom feature がないため placeholder が正しい。LiDAR や state の図ではない|
...
|closed-loop `navigation`|`mean_delta_total_reward = -106.549`、`mean_delta_success = -1`、`mean_delta_out_of_road = +1`|この 1 paired episode では、navigation 10 次元の入力置換後に報酬低下、成功減少、逸脱増加が観測された|
...
この run からの適切な暫定結論は、「この一つの seed では、episode-start replacement に対して speed と navigation が固定 PPO 方策の actor 出力を変え、selected targets の closed-loop 置換では走行成績も悪化した。だが 1 scenario、phase 未定義、入力置換による分布外化の可能性があるため、道路一般・交通一般・因果一般へは拡張しない」です。

## 出力が多い理由と配布範囲

大量の出力は通常の報告にすべて必要なわけではありません。役割ごとに分けます。

|配布層|受け手|含めるもの|目的|
|---|---|---|---|
|報告本文|意思決定者・発表聴衆|実験条件表、selected CSV 数値、closed-loop の両側値と差分、必要な PNG 2–4 枚、限界|結論を誤解なく伝える|
|数値根拠|査読者・共同研究者|`analysis_metadata.json`（共有用は path を伏せた複製）、`feature_schema_expanded.csv`、5 本の summary CSV、`closed_loop_summary.csv`、必要なら `closed_loop_runs.jsonl`|数値・filter・index の追跡|
|再解析・監査|再現担当|上記に加え config/schema/checkpoint の hash と許可された checkpoint、`rollout_arrays.npz`、`rollout_steps.jsonl`、`rollout_metadata.json`（外部共有用は path を伏せた複製）、`perturbation_feature_steps.npz`、`ig_attributions.npz`、`closed_loop_runs.jsonl`|別集計、plot 再作成、run の追跡|

`rollout_arrays.npz` と rollout metadata/steps は、MetaDrive を再走行せず `analyze` で保存済み observation を再解析する基礎です。摂動・IG の NPZ は per-step/per-target の監査や別集計に必要です。一方で、通常の PDF やスライドへ巨大な NPZ を添付する必要はありません。削除ではなく、報告用 bundle と監査用 archive を分けます。
