...
|---|---|---|---|
| pre-action（行動前） | 注視点の正規化座標 2 値と、注視点を利用できるかを示すフラグ | **今回の入力（観測）**。 | [1.1 観測の構成](#11-入力と出力)、[1.6 正規化](#16-10-m-基準の-clip-と正規化) |
...
| post-action（行動後） | 注視点の正規化座標 2 値と、注視点を利用できるかを示すフラグ | **次回の入力（観測）**。 | [1.6 正規化](#16-10-m-基準の-clip-と正規化) |
...
Navigation の計画経路が reset 時から変わった場合も、注視点を無効にします。
...
$b_t$ は、**1.3〜1.5 節で説明した、注視点を利用できるかの判定結果**を、観測用の数値にしたものです。
